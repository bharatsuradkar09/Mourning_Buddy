...
def get_weather(city_name:str):
    """
    fetching current weather for a give city using API
    args:
    city(str) : city name (e.g Delhi)
    ...
    try :
        api_key = '41653af25b43d9794c28d4b28aa53815'
...
#FUNCTION TO GET LATEST NEW HEADLINE
def get_news(topic : str):
    """
    fetching latest news and headlines from API
    args:
    topic(str) : topic top search news (e.g-technology,cricket etc.)
    """

    try :
        api_key = '0161ae804e9b42d6a2f78c8c7dc46947'
...
#FUNCTION OF NEWS SUMMARIZER
def news_summarizer(url : str):
    response = client.models.generate_content(
        model='gemini-2.5-flash',
        contents =f"summarize news from the url:- {url}, dont add sentences like from where the articles is ,in this article etc. Just give clear and crisp summary with one paragraph."
...
def get_forecast_weather(city:str):
    """
    LLM TOOL : Fetches forecasted weather and tourist places to visit given by city.
    args:
      city(str) : city name(e.g Delhi,Pune etc.)
...
def find_local_events(city : str):
    """
    find local events for given city using an API
    args:
     city(str) : city name (Pune,mumbai etc.)

    """
    try:

        api_key = '71320c49184583fc0d1276ac58ba8138d4b283f22ae04a8a7d473692df5d9a70'
...
def format_events(events_data, limit=5):
    """
    Clean and format SerpAPI events JSON into readable text.
    Removes duplicates and limits results.
    """
    if "events_results" not in events_data:
        return "No events found."

    seen = set()
    formatted = []

    for ev in events_data["events_results"][:limit]:
        title = ev.get("title", "Event")
        date = ev.get("date", {}).get("when", "")
...
# def smart_plan(city):
    ...
    response= client.models.generate_content(
        ...
        contents=prompt,
        config= types.GenerateContentConfig(
            tools=[find_local_events,get_forecast_weather]
        )
    )

    return response.candidates[0].content.parts[0].text
def smart_plan(city, time_window="9:00 AM – 9:00 PM"):
    weather_places = get_forecast_weather(city)
    events_data = find_local_events(city)
    events_text = format_events(events_data)

    prompt = f"""
    You are a smart travel and event planner assistant.
    Create a personalized day itinerary for {city}.
    ...
    Instructions:
    - Use weather conditions to decide indoor vs outdoor activities.
    - Organize plan: Morning → Afternoon → Evening.
    - Mix sightseeing, events, and meal breaks.
    - Always include event links.
    - If multiple events overlap, present them as choices (but no duplicates).
    """
