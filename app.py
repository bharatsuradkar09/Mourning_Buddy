...
def weather_news_page():
    ...
    if st.button('Fetch Information'):
        if city:
            temparature_output = temperature_of_city(city)
            st.subheader(f"Weather Info : {temparature_output}")
            st.success("Weather fetch successfully")
        else:
        ...
        if interest:
            ...
            for i in range(len(article)):
                with columns[i]:
                    ...
                    st.markdown("---")
                    st.write("Read a full article here: ",url[i])
                    st.markdown("---")
                    st.write(news_summarizer(url[i]))
        else :
            st.error('Please enter area of interest')
...
def smart_planner():
    ...
    city = st.text_input("Enter your city name:")
    if st.button('Lets Plan'):
        if city:
            smart_planer = smart_plan(city)
            st.subheader(smart_planer)
